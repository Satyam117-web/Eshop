      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            <h3 className="text-2xl font-bold text-blue-600">E-Shop</h3>
        ...
        <div className="border-t border-gray-900 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="text-gray-900 text-sm">
              <p>Secure payments • 24/7 Support • Free shipping over ₹500</p>
            </div>
          </div>
        </div>
      </div>
...
function SocialIcon({ href, icon }: { href: string; icon: string }) {
  const icons: { [key: string]: string } = {
    ...
    twitter: 'M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723 10.016 10.016 0 01-3.127 1.184 4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z',
...
function FooterLink({ href, children, small = false }: { href: string; children: React.ReactNode; small?: boolean }) {
  return (
    <li>
      <a 
        href={href} 
        className={`text-gray-900 hover:text-white transition-colors ${small ? 'text-xs' : ''}`}
      >
        {children}
      </a>
    </li>
